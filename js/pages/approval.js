  ...
  (r.rows||[]).forEach(res=>{
    ...
    tbody.querySelectorAll('.sel-mess').forEach(sel=>{
      sel.addEventListener('change', async (e)=>{
        ...
        await populateRoomsSelect(tr.querySelector('.sel-room'), mess_alloc, /*preselect*/ '');
        // simpan alokasi mess
...
async function initRoomsForAllRows(tbody, res){
  ...
  for(const tr of rows){
    ...
    tasks.push(populateRoomsSelect(gRoomSel, messName, pre));
  }
  await Promise.all(tasks);
}

// Isi dropdown kamar sesuai mess; gunakan cache agar hemat request
async function populateRoomsSelect(selectEl, messName, preselect=''){
  ...
  // 1) coba isi normal
  let ok = await fillRoomsOnce(selectEl, rawMess, preselect);

  // 2) kalau masih kosong (misal ada spasi aneh), retry pakai trimmed sekali lagi
  if(!ok && rawMess){
    ok = await fillRoomsOnce(selectEl, norm(rawMess), preselect);
  }

  // 3) kalau tetap kosong, beri placeholder agar operator paham
  if(!ok){
    selectEl.insertAdjacentHTML('beforeend',
      `<option value="" disabled>(kamar tidak ditemukan untuk mess "${rawMess}")</option>`);
  }

  // Re-populate otomatis ketika user fokus ke dropdown kamar yang masih kosong
  selectEl.addEventListener('focus', async ()=>{
    if(selectEl.options.length <= 1){ // hanya "- pilih kamar -"
      await fillRoomsOnce(selectEl, norm(selectEl.closest('tr').querySelector('.sel-mess').value), '');
    }
  });
