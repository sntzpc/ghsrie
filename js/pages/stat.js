...
async function renderRoomCards(messName) {
  ...
  rooms.forEach(room => {
    const status = (room.status || '').toLowerCase(); // 'tersedia' | 'terisi' | 'penuh'
    ...
    div.innerHTML = `
      <div class="card stat-card shadow-sm h-100 ${cardClass}">
        <div class="card-body d-flex flex-column">
          ...
          <div class="mt-1">Terpakai: <b>${used}</b> / <b>${cap}</b></div>
    ...
    if (btnPesan) {
      btnPesan.addEventListener('click', () => {
        try {
          ...
          showNotif(`Kamar dipilih: ${room.mess_name} - ${room.room_name}`);
    ...
    if (btnDetail) {
      btnDetail.addEventListener('click', () => {
        view = { level: 'guests', mess: room.mess_name, room: room.room_name };
        showStat();
      });
    }
