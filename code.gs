/***** ====== CONFIG ====== *****/
const SPREADSHEET_ID = '1t9v6kaaL7pPycoM-5FROMNdUEOCHQOcXDCVn5on2pyo';
...
function roomsList_(user, body){
  ...
  // Rooms: id, mess_name, room_name, capacity, grade, status, created_at, is_active
  var rvals = sheet('Rooms').getDataRange().getValues().slice(1);
  ...
  // hitung aktif (checkout_date kosong) per (mess,room)
  var occ = {}; // key: "mess||room" -> jumlah terpakai
  ...
  rvals.forEach(function(r){
    ...
    if(messFilter && mess !== messFilter) return;

    var key  = mess + '||' + room;
    var used = occ[key] || 0;

    var computed = 'tersedia';
    if(cap > 0 && used >= cap) computed = 'penuh';
    else if(used > 0)          computed = 'terisi';
    ...
    rows.push({
      ...
      capacity: cap,
      grade: grade,
      used: used,          // jumlah terpakai (aktif)
      status: computed     // 'tersedia' | 'terisi' | 'penuh'
    });
  });
